...
dataset_w = open(dataset_file, 'w',newline='')
writer = csv.writer(dataset_w)
writer.writerow(['Music_Title','Lead_Vocal', 'Backing_Vocal','Bass','Drum_Kick','Drum_Snare','Drum_HiHat','Drum_Cymbals','Drum_Overheads','Drum_Tom','Drum_Room','Percussion','Acoustic_Guitar','Electric_Guitar','Piano','Electric_Piano','Brass','String','WoodWind','Other','Unused'])
...
for ii, (root, dirs, files) in enumerate(os.walk(root_path)):
    try:
        ...
        Music_Title = root.split('\\')[1]
        Music_Title = Music_Title.split('_')[0]
